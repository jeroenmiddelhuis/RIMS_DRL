...
class Resource(object):
    ...
    def __init__(self, env: simpy.Environment, name: str, capacity: int, calendar: dict, start: datetime, type):
        self.env = env
        self.name = name
        ...
        if type == 'FIFO':
            ...
            self.resource_simpy = simpy.PriorityResource(env, capacity)
        self.start = start
        self.queue = []
    ...
    def request_SPT(self, duration):
        self.queue.append(self.resource_simpy.queue)
        return self.resource_simpy.request(duration)
