...
def main(argv):
    opts, args = getopt.getopt(argv, "h:t:l:n:p:")
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            POLICY = arg
    print(NAME_EXPERIMENT, N_SIMULATION, type, POLICY)
    run_simulation(NAME_EXPERIMENT, N_SIMULATION, type, POLICY)
...
def setup(env: simpy.Environment, NAME_EXPERIMENT, params, i, type):
    ...
    path_result = NAME_EXPERIMENT + '/results/simulated_log_LSTM_' + NAME_EXPERIMENT + '_' + str(i) + '_' + params.POLICY + '.csv'
    f = open(path_result, 'w')

    writer = csv.writer(f)
    writer.writerow(['caseid', 'task', 'enabled:time', 'start:timestamp', 'time:timestamp', 'role', 'st_wip', 'st_tsk_wip', 'queue'])
...
def run_simulation(NAME_EXPERIMENT, N_SIMULATION, type, POLICY):
    path_model = NAME_EXPERIMENT + '/' + NAME_EXPERIMENT
    print(path_model)
    ...
    for i in range(0, N_SIMULATION):
        params = Parameters(NAME_EXPERIMENT, FEATURE_ROLE, i, type, POLICY)
...
# python main.py -t rims -l BPI_Challenge_2012_W_Two_TS -n 1 -p FIFO

# python main.py -t rims -l ConsultaDataMining201618 -n 1 -p FIFO

# python main.py -t rims -l confidential_1000 -n 1 -p FIFO
